...
float worldSDF(vec3 p) {
	/* vec3 pos = pc.pos; */
    /* p = mod(p * pos.x, 2) + pos.y; */

    vec3 p2 = p;
    p2.xy = rotate(p.xy, PI / 2);
    float torus = torusSDF(p2, vec2(1., 0.25));

    float box = boxSDF(p, vec3(-0.31, 1.31, 1.02));
...
void main() {
	...
	/* uv *= 1. - length(rotate(uv, pc.time) + 0.5) / 3; */
    ...
    mat3 k = mat3(pos, pos, pos) - mat3(EPS.y);
    norm = normalize(worldSDF(pos) -
                     vec3(worldSDF(k[0]), worldSDF(k[1]), worldSDF(k[2])));

    vec3 col = vec3(0.);
    vec3 light = normalize(vec3(1.));
	float bounce = dot(norm, light);
    float brightness =
        mix(max(bounce, 0), bounce * 0.5 + 0.5, 0.2);
	/* brightness /= 2; */
    if (d > 0) {
        col = norm * 0.5 + 0.5;
        col = vec3(brightness);
    ...
    /* out_color = vec4(col / (1.0 + col), 1.0); */
}
